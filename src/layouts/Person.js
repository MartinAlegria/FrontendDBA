  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (props.match.params.type !== "Director") {
        personMovies = await (
          await fetch(
            `http://localhost:9000/actorsMovies/${props.match.params.id}`
          )
        ).json();
      } else {
        personMovies = await (
          await fetch(
            `http://localhost:9000/directorsMovies/${props.match.params.id}`
          )
        ).json();
      }

      const actorInfo = await (
        await fetch(`http://localhost:9000/actorInfo/${props.match.params.id}`)
      ).json();
      let temp = [];
      console.log(actorInfo);
      personMovies.forEach((per) =>
        temp.push({
          title: per.titulo,
